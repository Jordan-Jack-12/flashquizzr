...
import { generateFlashcardFromUrl } from "@/actions/generate/Flashcards";
...
const UrlLinkForm = () => {
  ...
  const handleGenerate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setLoading(true);
      const response = await generateFlashcardFromUrl(formData);
      if (response.errors || response.success === false) {
        toast.error(
          `${response.errors?.aiModel![0]}, ${response.errors?.prompt![0]}, ${response.errors?.type![0]}, ${response.errors?.noOfCards![0]}`,
        );
        return;
      }
  ...
  return (
    <div>
      <form
        ...
        className="m-2 flex flex-col justify-end gap-2"
      >
        <textarea
          placeholder="Type or Enter URL here..."
        ...
        <div className="text-sm flex gap-2 justify-end">
          ...
          <button
            disabled={loading}
            type="submit"
            className="button_gradient px-4 py-2 rounded text-orange-950 font-bold "
          >
            Generate
          </button>
        </div>
      ...
      {cards && cards.length > 0 && (
        <>
          ...
          <input
            type="text"
            className="block w-full text-2xl font-semibold outline-0"
            value={name || ""}
            onChange={(e) => setName(e.target.value)}
          />
          <input
            type="text"
            className="block w-full text-base text-stone-400 outline-0"
            value={description}
          ...
          {cards.map((item, index: number) => {
            return (
              <CardItem
                ...
                handleEditFront={handleEditFront}
      ...
      {cards && cards.length > 0 && (
        <div className="sticky bottom-0 right-0 left-0 flex justify-between text-sm bg-white dark:bg-stone-900 py-2 ">
          <button
            className="bg-stone-800 p-2 rounded cursor-pointer"
            onClick={handleScrollOnGenerate}
          >
            <ArrowUpFromLine size={16} strokeWidth={2} />
          </button>
          <button
            className="flex gap-2 button_gradient py-2 px-4 rounded text-orange-950 font-bold cursor-pointer"
            onClick={handleSave}
          >
            Save
          </button>
